...
public sealed class PersistRevisionInProgressIssue : IAsyncDisposable
{
    private readonly SqliteConnection _connection;
    private readonly FoundryDbContext _dbContext;

    public PersistRevisionInProgressIssue()
    {
    ...
    [Fact]
    public async Task WhenRevisionInProgressFromRevisionQueued_CanBeReloadedWithAllFields()
    {
        // Arrange
        ...
        RevisionInProgressIssue revisionInProgress = new IssueBuilder()
            ...
            .WithPullRequestUrl("https://github.com/owner/repo/pull/20")
        ...
        Issue? result = await _dbContext
            .Set<Issue>()
            .FindAsync([revisionInProgress.Id], TestContext.Current.CancellationToken);

        // Assert
        RevisionInProgressIssue reloaded = result.ShouldBeOfType<RevisionInProgressIssue>();
        reloaded.ShouldSatisfyAllConditions(
            () => reloaded.WorkerRunId.ShouldBe(workerRunId),
            () => reloaded.BranchName.ShouldBe("feat/issue-60"),
            () => reloaded.PullRequestUrl.ShouldBe("https://github.com/owner/repo/pull/20"),
