...
- **Guaranteed in-process teardown.** `LoginSessionService.SubmitCodeAsync` tears the container down in a `finally`, so every exit path — success, invalid code, timeout, host shutdown — removes it exactly once. A second remove is a safe no-op: `RemoveAsync` swallows both `DockerContainerNotFoundException` (404, container already gone) and the `AutoRemove` race where Docker returns 409 Conflict "removal … already in progress" — both mean the container is being or has been removed, which is the desired end state. This idempotency is what lets explicit teardown compose with `AutoRemove` without surfacing a spurious failure.
- **Two-layer reaping, state-scoped.** Both reapers list by `foundry.transient=true`. `LoginContainerReaper` (startup, `IHostedLifecycleService`) reaps transient containers of *any* state — safe because it runs before the app accepts requests, so no session is live and any running transient container is a guaranteed crash orphan. `TransientContainerReaper` (periodic `PeriodicBackgroundService`, 60 s) reaps only *exited* transient containers — a running transient container is either the active login, an in-flight helper, or an orphan already bounded by `timeout`+`AutoRemove`, so the periodic sweep excludes running containers and only reclaims the residue when `AutoRemove` fails to delete an exited container. Each reap tick is wrapped in `try/catch (Exception ex) when (ex is not OperationCanceledException)` — an unhandled throw would trip `BackgroundServiceExceptionBehavior.StopHost` and kill the host, the exact failure mode being fixed.
